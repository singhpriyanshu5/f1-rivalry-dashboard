-- Sprint head-to-head: teammate comparison on sprint race results
-- Mirrors mart_race_h2h but reads from stg_sprint_results
-- Only produces rows for sprint rounds (non-sprint rounds have no data)

with results_with_dnf as (
    select
        r.*,
        coalesce(d.category, 'finished') as dnf_category
    from {{ ref('stg_sprint_results') }} r
